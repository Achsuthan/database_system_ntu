/*
    ...
    Assumption: User already registered on the money manager system and friend with other users each other.
...
from friend
where (friend.receiver_userId = user.user_id AND friend.is_Friends = true) 
OR (friend.sender_userId = user.user_id AND friend.is_Friends = true)
);